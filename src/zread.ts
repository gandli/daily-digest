...
export async function fetchZreadWikiDesc(repo: string, maxLen = 280): Promise<string | null> {
  try {
    // zread.ai 2026-08 起变慢/风控。尽力而为: 短超时, 失败即落 deepwiki/翻译——不让它拖慢整条管线
...
/** 批量抓取——并行, 短超时。 */
// ponytail: 曾改串行限速, 但把整条管线拖死(zread 慢时 10×15s=150s, 长连接被掐)。zread 慢=站点全局问题, 并发不背锅;
// 改回并行+短超时, 让 zread 快速成功或快速失败落 deepwiki, 保整管线 <60s
