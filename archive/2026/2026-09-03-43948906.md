# Web Archive · https://huggingface.co/blog/train-to-paint-with-code

Title: Training a coding model to paint watercolours with TRL and OpenEnv

URL Source: https://huggingface.co/blog/train-to-paint-with-code

Published Time: 2026-09-03T00:00:00.740Z

Markdown Content:
[![Image 1: Training a coding model to paint watercolours](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/train-to-paint-with-code/thumbnail.png)](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/train-to-paint-with-code/thumbnail.png)
...
[Video 3](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/train-to-paint-with-code/three-runs-evolution.mp4)
...
Here the reward is aesthetic preference. There is no _correct_ answer. The real question of the project is whether you can do RL over taste.
...
![Image 2: The reward pipeline, piece by piece](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/train-to-paint-with-code/reward-diagram.png)
...
I started with `hps-only` to validate that the pipeline could learn at all. Once the reward was going up and the metrics were healthy, there was no reason to run it longer, so I launched the two longer runs instead. The question that the longer runs ask is how much of HPSv3's power can you hand to the pairwise judge? The more weight the judge carries, the more the reward means _my_ taste instead of everyone's, and the harder it should be to climb. Incidentally, if you push it far enough or your style is too far from the average, the model could stop entirely.
...
> **Note.** The author of p5.brush had been trying to teach a machine to paint long before any of this. In 2022 he made a generative art series that hides a diary about teaching p5.js to draw like a child: _"It is barely able to use the crayons [...] It cannot follow simple commands. I'm done for today, very infuriating."_ The series was meant to have three pieces, and he made two. When Surya's video went viral, [he quoted it](https://x.com/acamposuribe/status/2091668313449316651), sharing that diary and saying this work is the third piece arriving on its own.
...
![Image 3: A generated sketch and the painting it produces](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/train-to-paint-with-code/sketch-and-render.png)
...
![Image 4: A love reference beside an okay one](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/train-to-paint-with-code/love-and-okay.png)
...
![Image 5: Three reward experiments flat against the run that worked](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/train-to-paint-with-code/four-curves.png)
...
| setting | from | to | why |
| --- | --- | --- | --- |
| learning rate | 2e-5 | **5e-5** | the ceiling _LoRA Without Regret_ uses for GRPO |
...
![Image 6: The three runs, one curve each](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/train-to-paint-with-code/three-mixes.png)
...
![Image 7: Median against best, per step](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/train-to-paint-with-code/median-vs-best.png)
...
![Image 8: 178 paintings from the two judge runs, unlabelled](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/train-to-paint-with-code/pick-your-favourites.png)
...
![Image 9: The median painting of the last step of each run](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/train-to-paint-with-code/three-styles.png)
...
![Image 10: Steps 11 and 12 of the judge-led run, best four of each](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/train-to-paint-with-code/step-11-vs-step-12.png)
...
![Image 11: The infrastructure: what is billed during a run, and what outlives it](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/train-to-paint-with-code/infra-diagram.png)
