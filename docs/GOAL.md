...
状态：v1.4 —— 反映 2026-08-26 已实现功能（搜索/分页/三链/多源存档）。
...
- 同一 Worker 提供命令与链接处理：`/trending`（当日榜单）、`/search`（星标/书签/存档全索引搜索）、`/archive`（历史存档分页）、`/help`；
- 任意 GitHub repo / X 帖 / 网页 URL 链接 → 提取中文摘要并**三级存档**（Telegraph → 互联网档案馆 web.archive.org → GitHub md）；
- 每日及每次单仓查询自动 commit 至本仓 `archive` 分支。
...
- F5 链接处理：GitHub repo → `lookupRepo`（首次存档，当日已查 `seenToday` 命中回存档三链）；X 帖 → `archiveTweet`（FxEmbed + 摘要）；任意 URL → `archiveUrl`（markdown 三级链 + 摘要 + 存档）。
...
**管线**
- F9 sources 注册表：`src/sources/index.ts` 导出 `Source[]`，`SourceItem = { title, desc, url, lang?, extra? }`。
- F10 trending source：`fetchTrending` HTMLRewriter 解析 github.com/trending top10。
- F11 翻译/摘要：`translateBatch` 四级回退（Workers AI → MyMemory → TranSmart → 保留原文）；`summarizeZh` 用 CF `@cf/facebook/bart-large-cnn` 摘要 → m2m100 译中（长文/X 帖）。
- F12 渲染：每仓一条 sendPhoto（OG 图 + caption）；头部日期 + `#digest`；单条 ≤4096。
- F13 cron：`crons:["30 0 * * *"]`，scheduled 执行完整管线。
- F14 缓存：KV `digest:<date>` 存 `{chunks, repos}`（重放带 OG 图）；写失败不影响已发消息。
- F15 存档：Contents API PUT 到 archive 分支（幂等，同日覆盖）；`archive:<YYYY>` 分级目录；OG 图入库 `og-images/`。
- F16 Telegraph：digest 与 X 帖建页后写 `archive:tg:<date>`（digest，每日一条）与 `archive:tg:<stamp>`（X 帖，每帖唯一）；`/archive` 与存档卡优先展示 Telegraph。
- F17 web.archive：存档三链含 `web.archive.org/web/2/<source_url>`（有源 URL 时）。`archiveLinks` helper 统一 Triple 渲染。
- F18 搜索索引：`search:index` 单键压缩索引（seed 脚本从 library.jsonl 生成）；CI deploy 后播种。存档写入时 `indexArchivedItems` 增量维护 `archive:idx:<repo>`。
...
| A13 | `npm test` 全绿（139 tests）| CI |
| A14 | `npx vitest run --coverage` 整体 ≥45% | 本地 |

## Constraints

- C1 全免费层：Workers/KV/Workers AI/TranSmart/MyMemory 用量在免费额度内；代码禁止付费服务凭证。
- C2 secrets(BOT_TOKEN/CHAT_ID/GH_TOKEN/WEBHOOK_SECRET/TELEGRAPH_TOKEN)只经 `wrangler secret put`；不得出现在源码/配置/日志。
...
- M5（计划）：描述缓存 refresh 调优、search 翻译结果缓存回索引、archive:idx 补 url 字段